...
class Control_Core extends Base_Data_Object {
	...
	'render_content'() {
		...
		if (tof(content) === 'string') {
			...
			// Handle both Collection (has .length() method) and arrays (have .length property)
			var contentLength = (typeof content.length === 'function') ? content.length() : content.length;
			var res = new Array(contentLength);
			var tn, output;
			var arr = content._arr || content; // Support both Collection and plain arrays
			var c, l = arr.length,
				n;
			for (c = 0; c < l; c++) {
				n = arr[c];
